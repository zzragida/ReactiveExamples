...
namespace HistoricalScheduling
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            log.Process((xs, s) =>
            {
                ...
                var res3 = Observable.Interval(TimeSpan.FromDays(1), s)
                                .TakeUntil(new DateTimeOffset(2013, 1, 1, 12, 0, 0, TimeSpan.Zero), s)
                                .Select(_ => s.Clock);
                res3.Subscribe(t =>
                {
    ...
    class LogScheduler<T>
    {
        ...
        class Scheduler : HistoricalScheduler
        {
            ...
            public void MoveNext(bool initializeIntialTimeFromLog = false)
            {
                var nextLog = default(Timestamped<T>);
                if (TryMoveNext(out nextLog))
                {
                    if (initializeIntialTimeFromLog)
